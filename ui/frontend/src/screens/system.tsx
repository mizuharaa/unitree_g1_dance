...
function SystemOverview({ data }: { data: ConsoleData }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-4 xl:grid-cols-[1fr_360px]"><Card><CardHeader className="flex-row items-center justify-between space-y-0"><div><div className="panel-kicker"><Cpu /> Training monitor</div><CardTitle className="mt-2">Active workloads</CardTitle></div><Badge variant={jobs.length ? "success" : "secondary"}>{jobs.length ? "live" : "idle"}</Badge></CardHeader><CardContent>{jobs.length ? <div className="space-y-3">{jobs.map((job) => { const pct = job.iteration && job.max_iteration ? job.iteration / job.max_iteration * 100 : 0; return <div key={job.name} className="rounded-lg border border-border bg-background/25 p-4"><div className="flex items-center justify-between gap-2"><div className="text-xs font-semibold">{job.name}</div><StatusBadge status="running" /></div><Progress className="mt-3" value={pct} /><div className="mt-2 flex flex-wrap justify-between gap-2 font-mono text-[10px] text-muted-foreground"><span>{job.iteration?.toLocaleString() ?? "—"} / {job.max_iteration?.toLocaleString() ?? "—"}</span><span>reward {job.mean_reward?.toFixed(2) ?? "—"}</span><span>ep {job.mean_episode_length?.toFixed(0) ?? "—"}</span></div></div>})}</div> : <EmptyState icon={Cpu} title="No active training" body="The current box snapshot reports no running trainer." />}</CardContent></Card><Card><CardHeader><div className="panel-kicker"><Gauge /> Budget</div><CardTitle className="mt-2">Spend guardrail</CardTitle></CardHeader><CardContent><div className="flex items-end justify-between"><span className="text-3xl font-semibold">{fmtMoney(cost?.accrued_vnd)}</span><span className="text-xs text-muted-foreground">cap {fmtMoney(cost?.cap_vnd)}</span></div><Progress className="mt-4 h-3" value={Math.min(100, cap)} indicatorClassName={cost?.over_cap ? "bg-red-500" : "bg-blue-500"} /><div className="mt-2 flex justify-between text-[10px] text-muted-foreground"><span>{cap.toFixed(1)}% consumed</span><span>{cost?.over_cap ? "OVER CAP" : "within cap"}</span></div><div className="mt-5 rounded-lg border border-amber-500/20 bg-amber-500/[.06] p-3 text-[10px] leading-5 text-amber-100/70">GreenNode billing ends only when the instance is deleted—not when it is stopped.</div></CardContent></Card></div>
  </div>
}
