...
import { AlertOctagon, ArrowDown, ArrowUp, Check, ChevronRight, CircleAlert, Footprints, GripVertical, ListMusic, MapPin, Plus, Radio, ShieldAlert, ShieldCheck, Sparkles, Trash2, Users, Volume2, WifiOff, X } from "lucide-react"
...
function InlineRunGate({ dance, blocked }: { dance: Dance; blocked: boolean }) {
  ...
  return <div className="rounded-xl border-2 border-red-300 bg-red-50 p-4 shadow-sm" data-testid="show-warning-gate">
    ...
    <div className="mt-3 flex items-start gap-3 rounded-lg border border-red-300 bg-white p-3" data-testid="ground-contact-warning"><Footprints className="mt-0.5 h-5 w-5 shrink-0 text-red-600" /><div className="min-w-0"><div className="text-xs font-black text-red-900">Feet flat on the ground before you run</div><p className="mt-1 text-[11px] leading-5 text-red-800/80">A robot hung in the air or barely touching the floor will thrash violently in a policy mode — it can’t find the ground it’s balancing on. Plant both feet on solid flat ground first. If it thrashes, E-STOP + remote B-damp immediately.</p></div></div>
