...
import { BarChart3, ChevronRight, CircleAlert, Clapperboard, Command, Cpu, History, LayoutDashboard, Menu, Moon, Radio, RefreshCw, Shield, ShieldAlert, Square, Sun, Workflow, X } from "lucide-react"
...
import { EStopButton, robotStateFrom } from "@/components/robot-state"
...
const SafetyScreen = lazy(() => import("@/screens/safety").then((module) => ({ default: module.SafetyScreen })))

type Screen = "overview" | "pipeline" | "dances" | "simulation" | "safety" | "perform" | "audit" | "system"
...
const NAV: Array<{ id: Screen; label: string; short: string; icon: typeof LayoutDashboard; section: "mission" | "operate" | "evidence" }> = [
  ...
  { id: "safety", label: "Safety & E-stop", short: "Safety", icon: ShieldAlert, section: "operate" },
...
function Sidebar({ active, onNavigate, data }: { active: Screen; onNavigate: (screen: Screen) => void; data: ReturnType<typeof useConsoleData> }) {
  return <aside className="fixed inset-y-0 left-0 z-40 hidden w-[236px] flex-col border-r border-slate-200 bg-white px-3 py-4 lg:flex"><div className="px-2"><Brand /></div><div className="mt-8 flex-1">{(["mission", "operate", "evidence"] as const).map((section) => <div key={section} className="mb-6"><div className="mb-2 px-3 text-[9px] font-bold uppercase tracking-[.17em] text-slate-400">{section}</div><nav className="space-y-1">{NAV.filter((item) => item.section === section).map((item) => { const Icon = item.icon; const count = item.id === "dances" ? data.dances.length : item.id === "audit" ? data.shows.length : undefined; const safetyState = robotStateFrom(data.run); return <button key={item.id} onClick={() => onNavigate(item.id)} className={cn("group flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-xs font-semibold transition-all", active === item.id ? "bg-blue-50 text-blue-800 ring-1 ring-inset ring-blue-200 shadow-sm" : "text-slate-600 hover:-translate-y-px hover:bg-slate-50 hover:text-slate-900")}><Icon className={cn("h-4 w-4", active === item.id ? "text-blue-600" : "text-slate-400 group-hover:text-blue-500")} /><span className="min-w-0 flex-1">{item.label}</span>{item.id === "safety" && safetyState === "fall" && <span className="estop-pulse h-2.5 w-2.5 rounded-full bg-red-500" title="Fall / instability detected" />}{item.id === "safety" && safetyState !== "fall" && data.run.running && <span className="h-2.5 w-2.5 animate-pulse rounded-full bg-blue-500" title="Run active" />}{count != null && <span className="rounded-full bg-slate-100 px-1.5 py-0.5 font-mono text-[9px] text-slate-500">{count}</span>}{active === item.id && <ChevronRight className="h-3 w-3 text-blue-500" />}</button>})}</nav></div>)}</div><div className="space-y-2 border-t border-slate-200 pt-3"><div className="rounded-lg bg-slate-950 p-3 text-white shadow-lg"><div className="flex items-center gap-2"><span className="text-[10px] font-semibold">GPU box</span><span className="ml-auto text-[9px] text-slate-400">{data.system?.reachable ? "Live" : "Offline"}</span></div><div className="mt-3 flex justify-between text-[9px] text-slate-400"><span>Cloud spend</span><span className={cn("font-mono font-semibold text-white", data.system?.cost?.over_cap && "text-red-300")}>{fmtMoney(data.system?.cost?.accrued_vnd)}</span></div><div className="mt-2 h-1 overflow-hidden rounded-full bg-slate-800"><div className={cn("h-full rounded-full", data.system?.cost?.over_cap ? "bg-red-500" : "bg-blue-500")} style={{ width: `${Math.min(100, (data.system?.cost?.cap_fraction ?? 0) * 100)}%` }} /></div></div><div className="flex items-center gap-2 px-2 text-[9px] text-slate-400"><Command className="h-3 w-3" /><span>Local engine • :8735</span></div></div></aside>
}
...
function TopBar({ active, onMenu, data, theme, onToggleTheme }: { active: Screen; onMenu: () => void; data: ReturnType<typeof useConsoleData>; theme: "light" | "dark"; onToggleTheme: () => void }) {
  ...
  return <header className="sticky top-0 z-30 flex h-16 items-center border-b border-slate-200 bg-white/92 px-4 backdrop-blur-xl sm:px-6 lg:px-8"><Button variant="ghost" size="icon" className="mr-2 lg:hidden" onClick={onMenu} aria-label="Menu"><Menu /></Button><div className="hidden items-center gap-2 text-[11px] text-slate-500 sm:flex"><Shield className="h-3.5 w-3.5 text-blue-600" /><span>Operator console</span><ChevronRight className="h-3 w-3" /><span className="font-semibold text-slate-800">{item.label}</span></div><div className="text-sm font-semibold sm:hidden">{item.short}</div><div className="ml-auto flex items-center gap-2 sm:gap-3"><EStopButton compact /><div className="hidden rounded-full border border-slate-200 bg-slate-50 px-3 py-1.5 text-[10px] text-slate-600 md:flex">Engine {data.error ? "degraded" : "online"}</div><div className="hidden rounded-full border border-slate-200 bg-white px-3 py-1.5 font-mono text-[10px] text-slate-700 shadow-sm sm:block">{clock.toLocaleTimeString([], { hour12: false })}</div><Button variant="ghost" size="icon" onClick={onToggleTheme} aria-label="Toggle dark mode" title={theme === "dark" ? "Switch to light mode" : "Switch to dark mode"}>{theme === "dark" ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}</Button><Button variant="ghost" size="icon" onClick={() => data.refetchAll()} aria-label="Refresh data"><RefreshCw className="h-4 w-4" /></Button></div></header>
}
...
export default function App() {
  ...
  const screen = useMemo(() => {
    ...
    if (active === "safety") return <SafetyScreen data={data} />
